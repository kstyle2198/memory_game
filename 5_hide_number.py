...
def shuffle_grid(number_count):
    ...
    grid = [[0 for col in range(columns)]
            for row in range(rows)]  # 5 X 9 격자 그리기

    number = 1  # 시작숫자 1부터 number_count까지
...
def check_buttons(pos):
    ...
    elif start_button.collidepoint(pos):  # start button 내에 pos 가 위치하는지 확인
        start = True
...
number_buttons = []  # 플레이어가 눌러야 하는 버튼을 담을 리스트
curr_level = 1
display_time = None   # 숫자가 보여주는 시간
